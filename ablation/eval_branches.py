...
Variants (HybridCfg defaults, GRU):
  full         — all branches on
  no_graph     — use_graph=False
  no_temporal  — use_temporal=False
  no_pair      — use_pair=False

Only the *model* seed varies (42–44 by default); EvalConfig stays at 42 so
every run is scored on the same candidate set. Full-model numbers are measured
on this machine — do not copy CUDA reference numbers into the delta.

Usage:
  python eval_branches.py              # 4 variants × 3 seeds
...
def run_variant(variant: str, seed: int, data) -> dict:
    import hybrid_core as hc
    overrides = dict(next(o for n, o in VARIANTS if n == variant))
    cfg = hc.HybridCfg(seed=seed, **overrides)
...
def _load_done() -> set:
    ...
    return {(r["variant"], int(r["seed"])) for _, r in d.iterrows()}
...
def summarize(df: pd.DataFrame) -> pd.DataFrame:
    metric_cols = [c for c in df.columns if c not in ("variant", "seed")]
...
def write_comparison_md(summ: pd.DataFrame, n_seeds: int, device: str,
    ...
    summ = summ.set_index("variant").reindex(order).reset_index()
    full = summ[summ["variant"] == "full"].iloc[0]
    ...
    lines = [
        ...
        "",
        f"{len(order)} variants × {n_seeds} seeds ({SEEDS[0]}–{SEEDS[n_seeds-1]}), "
        f"~{elapsed_min:.0f} min on {device}.",
    ...
    for _, r in summ.iterrows():
        ap = f"{r['test_ap_mean']:.4f} ± {r['test_ap_std']:.4f}"
        mse = f"{r['test_mse_mean']:.4f} ± {r['test_mse_std']:.4f}"
        f1 = f"{r['test_f1_mean']:.4f} ± {r['test_f1_std']:.4f}"
        mark = "**" if r["variant"] == "full" else ""
        lines.append(
            f"| {mark}{r['variant']}{mark} | {mark}{ap}{mark} | {mse} | {f1} | "
            f"{r['test_ap_std']:.4f} |"
        )
    ...
    lines += [
        ...
        r"(√((σ_abl² + σ_full²) / 2)) — anything below ~2σ is not "
        "distinguishable from seed noise.",
        "",
        "AP and MSE can disagree: a branch may look irrelevant under binary AP "
        "while still contributing to the count head. Read both columns.",
        "",
...
def main():
    ...
    n_full = _nparams(hc.HybridCfg())
    for name, ov in VARIANTS:
        n = _nparams(hc.HybridCfg(**ov))
        assert n == n_full, f"{name}: {n} params != full {n_full}"
    print(f"param count identical across variants: {n_full}")
    ...
    for variant, _ in VARIANTS:
        print(f"\n--- {variant} ---")
        for s in seeds:
            if (variant, s) in done:
                print(f"  seed {s}: skipped (already in {os.path.basename(RAW_CSV)})")
                continue
            t0 = time.time()
            m = run_variant(variant, s, data)
            _append({"variant": variant, "seed": s, **m})
            print(f"  seed {s}: test_ap={m['test_ap']:.4f} test_mse={m['test_mse']:.4f}  "
                  f"({time.time()-t0:.0f}s)")
    ...
    summ = summ.sort_values("_ord").drop(columns="_ord")
