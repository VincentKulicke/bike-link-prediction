...
DEFAULT = {
    ...
    "LSTM":       dict(mse=0.116, mae=0.161, rmse=0.340),
}
...
def main():
    ...
    lines.append("> **Read together with `seeds_comparison.md`.** The numbers below are "
                 "single runs (seed 42) and carry no error bars. Across 5 seeds the "
                 "hybrid default-vs-HPO gap turns out to be seed noise, and for the LSTM "
                 "the grid now picks the default config, so there is no tuning gain to "
                 "report. Only the GraphMixer tuning gain survives (+0.106 AP, 3.4 σ). "
                 "GraphMixer is also by far the least stable model (±0.023 AP vs. ±0.0007 "
                 "for the hybrid), so its single-seed numbers should be read with that "
                 "spread in mind.\n")
    lines.append("> **LSTM numbers changed after a fix.** The baseline used to draw its "
                 "training windows uniformly from the raw count matrix (98.9% zeros, mean "
                 "0.013) while being scored on the 1:5 candidate set (83.3% zeros, mean "
                 "0.189). It therefore learned to predict near zero and ended up worse "
                 "than predicting the mean. It now trains on the same candidates it is "
                 "evaluated on: test MSE 0.233 -> 0.116.\n")
