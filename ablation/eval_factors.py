...
LSTM_CFGS = [     # (tag, lr, hidden_dim, num_layers)
    # re-picked from the grid after the training-distribution fix; the old
    # ranking came from a run that trained on a different distribution
    ("rank01_best",  1e-3,  64, 1),
    ("rank07",       1e-3, 128, 2),
    ("rank13",       3e-4,  32, 1),
