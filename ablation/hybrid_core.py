...
class HybridCfg:
    ...
    seed: int = 42
    # >0 enables early stopping on validation AP with this patience. Needed
    # once the lr grid is widened: with a fixed epoch budget a high lr and a
    # low one are not comparable.
    patience: int = 0
...
def run_hybrid(cfg: HybridCfg, data: HybridData,
    ...
    t0 = time.time()
    best, best_state, bad, stopped_at = -1.0, None, 0, cfg.epochs
    for ep in range(1, cfg.epochs + 1):
        ...
        for s in range(0, n, cfg.batch_size):
            ...
            tot += loss.item(); nb += 1
        msg = f"  epoch {ep:2d}/{cfg.epochs} | loss {tot/max(1,nb):.4f}"
        if cfg.patience > 0:
            ap = data.ev.score_binary(_predict(model, data, cfg, "val"), split="val")["ap"]
            msg += f" | val AP {ap:.4f}"
            if ap > best:
                best, bad = ap, 0
    ...
    res = {"cfg": cfg, "train_s": round(time.time() - t0, 1), "epochs_run": stopped_at}
