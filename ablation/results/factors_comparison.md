...
|---|---|---|---|
| rank 1 (best) | 1e-3, 64, 1 | 0.1275 | ±0.0004 |
| rank 7 | 1e-3, 128, 2 | 0.1278 | ±0.0007 |
| rank 13 | 3e-4, 32, 1 | 0.1303 | ±0.0005 |
| rank 18 (worst) | 1e-4, 32, 1 | 0.1345 | ±0.0013 |

Re-measured after the training-distribution fix; the configs were re-picked from
the corrected grid, so they are not the same four as before.
...
| LSTM | 0.0070 | 0.0071 | ~0 % |
| GraphMixer | 0.3383 | 0.2240 | **34 %** |

The LSTM row used to read 0.0250 / 0.0181 / 28 %. After the training-distribution
fix its seed noise dropped from ±0.0034 to ±0.0008, so its single-seed grid range
is no longer inflated — it now behaves like the hybrid.

## What this means

1. **The ratio σ_grid/σ_seed says how trustworthy a grid search can be at all.**
   For the hybrid (8.2×) and the LSTM (4.1×) hyperparameters clearly dominate the
   noise and the grid ranking is meaningful. For GraphMixer (1.3×) a single-seed
   grid search is barely informative.
