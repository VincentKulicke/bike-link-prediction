...
| LSTM | default | — | 0.1164 ± 0.0008 | — |

The LSTM has a single row because the grid search now selects `lr1e-3, h64, L1`,
which *is* the default — running an "HPO" variant would repeat the same config.
...
| LSTM: HPO vs. default | — | — | grid selects the default, nothing to compare |
...
| Hybrid vs. LSTM (MSE) | −0.034 MSE | ~54 | **decisive** |
...
2. **The LSTM baseline was fixed, and tuning no longer changes it.** The old
   numbers (0.240 default / 0.233 "tuned") came from a model that trained on the
   raw count matrix while being scored on the 1:5 candidate set — two very
   different distributions, so it learned to predict near zero and landed below
   a constant-mean predictor. Trained on the same candidates it is evaluated on,
   it reaches 0.1164 ± 0.0008, and the grid search now picks the default config.
   The old "tuning gain" was an artefact of optimising the wrong objective: the
   config it selected then (lr3e-4, h32, L1) ranks 13th of 18 today.
